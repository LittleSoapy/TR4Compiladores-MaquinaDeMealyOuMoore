package com.compiladores.trabalho.gamestates;
...
import com.compiladores.trabalho.logic.Problem;
import com.compiladores.trabalho.main.MaquinaDeMealyOuMoore;
...
public class PlayState extends GameState {
	...
	public void init() {
		...
		tiledMap = new TmxMapLoader().load("core/assets/maps/map1.tmx");
		//tiledMap = new TmxMapLoader().load("core/assets/maps/map2.tmx");
	...
	public void update(float dt) {
		// o timer no 60 � equivalente a 1 segundo de intervalo
		if (timer == timerLimit) {
	...
	public void draw() {
		...
		tiledMapRenderer.setView(MaquinaDeMealyOuMoore.camera);
		tiledMapRenderer.render();
		
		batch.begin();
        font.draw(batch, Passos, 5+0*32 , MaquinaDeMealyOuMoore.HEIGHT - 10);
        font.draw(batch, Comidas, (5+4*32)-16 , MaquinaDeMealyOuMoore.HEIGHT - 10);
        font.draw(batch, "Estado: " + problema.rato.estado, (5+8*32)+5 , MaquinaDeMealyOuMoore.HEIGHT - 10);
        
        if(problema.rato.estado == 0) {
            font.setColor(Color.RED);
            font.draw(batch, "ESTADO FINAL", 5+11*32  , MaquinaDeMealyOuMoore.HEIGHT - 10);
        }
        batch.end();
